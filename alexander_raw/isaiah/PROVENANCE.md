...
`scripts/isaiah_backlog.py` 按四类分：

```
已核实原样   4919   证人与我们一致 —— 底本就这么印的，不必再挖
外文/残渣    4599   希伯来/希腊/德文/拉丁 —— 页面上印的就是外文，
                    翻影像也变不出拉丁字母，除非真去转写
证人对不上    135   证人给了读数但与我们这串毫无相似之处
                    （`Carchemish → some`、`Ctesiphon → which`）——
                    那是锚落错地方，不是证人读崩；证人这条路已走到头
待判          572   剩下这些才值得翻影像
```

「证人对不上」要单独分出来，是诗篇线先提的（他们 435 条 divergent 里 211 条
属此类）。把它算进待判会让分母虚高，排期就是瞎猜。

572 仍是**上界**：里头还混着德文（`sondern` `zornig` `wandelt`）、拉丁
（`omnes` `pluralis`）与专名（`Libnah`），它们只是碰巧与某个英文词编辑距离
≤2 才落进来。真正可判的比这少。
