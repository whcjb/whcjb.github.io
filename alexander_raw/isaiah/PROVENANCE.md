...
判读跑完账上还有约 1.02 万个非词，但它们不是同一回事。
`scripts/isaiah_backlog.py` 按类分：

```
已核实原样          4919   证人与我们一致 —— 底本就这么印的，不必再挖
外文/残渣           4599   希伯来/希腊/德文/拉丁 —— 页面上印的就是外文，
                           翻影像也变不出拉丁字母，除非真去转写
待判·证人还有话说     212   证人给了形状接近的读数，只是没过闸
待判·证人对不上       135   读数与原串毫无相似之处（`Carchemish → some`、
                           `Ctesiphon → which`）—— 锚落错地方，不是读崩
待判·无证据           360   锚在九份证人里都没对上
                  ─────
                   10225

真正待判 707，其中 495 处（对不上 + 无证据）**只剩影像一条路**
```

两条口径都是跟诗篇线来回校准出来的：

- **「证人对不上」要单独分出来**（诗篇线先提）。把它算进「待判」会让分母
  虚高——证人这条路在那些位置已经走到头，翻影像也只是重新认一遍我们已经
  认得的字。
- **别用「有没有元音」区分残渣**（我先踩，诗篇线跟着复核）。`ia` `xal`
  `TOV` `rov` 全有元音，却都是希腊文音译。改成「ASCII、长度 ≥5、与本书
  某个出现 ≥5 次的正确词编辑距离 ≤2」，本书待判从 3066 收到 707，
  诗篇那边 296 收到 156。

**「对不上」与「无证据」两档同名不同义，取决于有几份证人。** 本书九份：
无证据是**票散了**——各份撞到不同位置，`poll` 要求两份给出同一读数且唯一
领先，撞车时自动作废，这一档正是多证人前置过滤起作用的痕迹；对不上是票没散
但读数离谱。诗篇单证人：无证据是**锚在证人语料里根本没落点**（两版之间那句
被改写过，或锚里的词全崩了），对不上是「证人有话但说错了」。看别的书卷的
账目时别把两边的同名档当成一回事。

707 仍是**上界**：里头还混着德文（`sondern` `zornig` `wandelt`）、拉丁
（`omnes` `pluralis`）与专名（`Libnah`），只是碰巧与某个英文词距离 ≤2。

**报非词率要带上「其中外文/残渣多少」**，否则会被读成处理质量的差距：
本书 1.61% 里 4599 处是外文与活字残渣（诗篇同一栏只有 635），把这一栏
从两边扣掉，可判部分的处理程度其实相当。
