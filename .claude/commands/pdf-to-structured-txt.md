...
**核心目标：原文是什么样的，就要在网页上复现出一样的效果。**

网页与 PDF 原文在结构、内容、视觉效果上必须完全一致。以下原则适用于所有 Calvin 注释书卷。

### 0. 最高原则：以 PDF 原文为唯一依据，禁止猜测

**原文是什么样的，就要在网页上复现出一样的效果。** 这是双向约束：

**正向**：PDF 里有的视觉效果，必须在网页上还原。
- 经文块是有边框的方框 → 网页必须输出 `<div class="scripture-box">` 带边框
- 经文引用是蓝色 → 网页必须用对应颜色
- 居中段落 → 网页必须居中
- 缩进引文 → 网页必须用 blockquote

**反向**：PDF 里没有的视觉效果，不得在网页上自创。
- 经文块和注释看起来一样 → 若 PDF 原文如此，就保持原样，不主动美化
- 不得因"觉得应该区分"就擅自加边框、背景色、缩进等 PDF 里没有的视觉效果
...
**反例（错误做法）**：
- 用户说右对齐不对 → 未看 PDF 就改成居中 → 结果 PDF 是左缩进，改了两次都错
...
### 0.3 全局原则：几何信号做语义判断必须搭配内容/样式信号校验

**问题模式**：用单一几何指标（行宽、x0、cx、bbox 大小等）判断**语义类别**（这是跨列吗？是经文吗？是新段吗？）几乎总会翻车。同一个几何特征可由多种原因导致，只有几何不够。

例（已踩过）：

- 单看「`line_w > col_slot_w × 1.4`」就判跨列，误把含上标脚注 ref 的全宽 commentary 行（如 `that is added to the principal:^418 whereas Christ...`）拆成两列
- 单看「`cx ≈ page_cx`」就判居中，误把刚好对齐的两端对齐 body 行判为居中
- 单看「`block.lines >= 3` 每列」就判多列，误把正文里偶有居中行的块当多列
...
**规则**：
- 任何用 bbox/坐标/宽度做 yes-no 分类的代码，写之前先问：「同样的几何还能由哪些**非目标场景**造成？」每种都列内容/样式排除信号
- 一行代码 `if line_w > X` 类的判定如果不挂内容校验，几乎一定会被反例打中
- 内容/样式信号包括但不限于：span flag（bold/italic/sup）、文本起首模式（`**N.**` / `Book Ch:N` / 数字开头）、行内 `*italic*` markdown、字体大小

PyMuPDF 给的 flag 位字段是**金矿**：
...
### ⚠️ 中文翻译产物必须强制保留（最高优先级，绝不可违反）

翻译耗时极长（每段单独 Claude CLI，一卷数百段 = 数十分钟），且依赖外部服务额度。**翻译产物丢失意味着重跑全量翻译，时间/成本极高。**

下列文件/目录**必须强制保留**，任何清理、重构、批量删除操作前置检查，禁止删除或在没有备份的情况下覆盖：

| 路径 | 内容 |
|---|---|
| `calvin_raw/<book>/zh_chapters/*.md` | 翻译后的章节 raw（如 harmony1）|
| `calvin_raw/<book>/calvin_<book>_zh.md` | 翻译后的单文件 raw（如 phil）|
| `calvin_raw/<book>/zh_cache/*.txt` | 每段增量缓存，断点续翻命脉 |
| `ocr_output/<book>/calvin_<book>_zh.md` | 旧版本路径下的同类产物 |

**操作规范：**
- 删除/重构 `calvin_raw/` 子目录前，必须 `find . -name "zh_*" -o -name "*_zh.md"` 列出受影响的翻译产物并报告用户
- 重新跑翻译脚本前，若 `calvin_<book>_zh.md` 已存在，必须先备份为 `.bak`
- 发布到站点目录（`calvin/<book>/N.md`）的中文文件是**派生品**，不能替代 raw；raw 才是真正的源头
- 若发现翻译产物意外丢失，立即停止后续动作，向用户报告并恢复
