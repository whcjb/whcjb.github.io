...
**每节经文的注释头不能只写节号** `**N.**`，必须写全**加粗**的「**书卷 章:节。**」，与对观福音
合参的 `**马太福音 4:1.**` 一致。**必须加粗**（`**...**`→`<strong>`）——`calvin-en` 布局 JS
靠行首 `<strong>` + `mZh` 中文模式识别注释头, 才给它加**可点击跳转顶部 verse-nav**并纳入
verse 导航；纯文本头不被识别、不可点、字体也不一致。
