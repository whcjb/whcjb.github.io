...
- [ ] ⛔ **要重跑已发布书卷的提取？先读 [§11.5](#115--当前阻塞重跑提取会丢经文2026-09-03-实测)。**
      当前 scripture-mode 已退化，重跑会**丢经文**（haggai 实测丢 1,145 词，
      jeremiah-1 丢 14.5 万字符）。已发布的 raw 比现在的产物更完整，
      重跑等于用坏代码覆盖好数据。必须先验「新 raw ⊇ 旧 raw」。
- [ ] 双语经文块审计：`python3 scripts/qa_bilingual_scripture.py --ot`
      （漏成散段 / 缺节 / 空单元格 三类；判据的四次踩坑见 §11.5 末尾）
...
### 11.6 空单元格的归因（2026-09-04 逐条查完，别再当 bug 修）

旧约双语表剩的空单元格不是同一种病，**其中一半以上是忠于原书的**。
逐条对着 PDF 查过，四类：

| 归因 | 处数 | 说明 |
|---|---|---|
| **中文页缺中文 → 须翻译** | 9 | 中文页的 `scripture-en` 装的是**中文**，PDF 里只有英/拉，**从 PDF 无从回填**。要补只能走 translate。工具必须拒绝这类，否则会把英文填进中文列（实测差点填错 jeremiah-2/40、/42、/44） |
| **AGES 该页未印此侧** | 6 | 原书那一节就只印了一侧（jeremiah-1-en/3 v1 拉丁、ezekiel/20 v1 拉丁中英、daniel-en/3 v5 英文、amos/3 v11 拉丁中英）。**空着才是忠于原书**，不该编造 |
| PDF 有该语种文本但取词逻辑未命中 | 1 | jeremiah-1-en/9 v24 英文 |
| box 无 ages-code，PDF 页定位失败 | 3 | harmony-law-1/{6,11}、hosea-en/1 |

### 回填器 scripts/recover_verse_from_pdf.py 的三条硬判别

写死这三条，缺一条就会填错（都实测踩过）：

1. **中文页只许补 `la`**。`en` 列是中文，PDF 取不到。
2. **语种必须对**。只按「与已有列不相似」挑，会把英文填进拉丁列
   （jeremiah-1/19 v4 的候选正是 `Because they have forsaken me…`）。
   用英/拉停用词计分钉死。
3. **唯一候选须与已有列相似度 <0.5**。否则说明找到的就是已有那侧，
   会把它抄回去（26 处因此被正确放弃，相似度多在 0.96–0.98）。

### PDF 取词的两个格式坑

- 节号后**可以没有空格**：`4.Propterea quod reliquerunt me…`（jeremiah p861）。
  正则写 `\.\s+` 会整节取不到。
- 节号可用**逗号**：`6, Ex dixit Jehova ad me`（jeremiah p534）。
- 缺侧常与 H2 或相邻节**合在同一块**里（英文经文与 `<241106>JEREMIAH 11:6-8`
  同块），只认「以节号起首的块」取不到，须再做块内切分。
