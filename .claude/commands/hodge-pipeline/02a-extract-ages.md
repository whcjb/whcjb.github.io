...
## 16. 行首编号：经文号 vs 列举项（贺智新增）

母 skill 的 `bold_leading_verse_num` 把**任何**行首 `N. ` 一律加粗
（`**1.** Foo`）。那既是加尔文各卷的排版约定（其 PDF 里节号本就是粗体），
也顺带打断 kramdown 对段首 `N. ` 的有序列表识别。

贺智不能照搬。PDF 实测：**1,045 处行首编号里只有 10 处是粗体**，经文号
`22.` 与列举项 `1.` **完全同字体同字号**（size 12、非粗体、黑色、同一 x0），
版面上唯一的区别是**后面跟什么**：

| 身份 | 后接 | 处数（1cor+2cor） | 网页呈现 |
|---|---|---|---|
| 经文号 | 红色经文引语 `<span style="color:#800000">` | 605 | **加粗** |
| 列举项 | 普通正文 | 193 | `<span class="enum-num">` 主题色 |

网页上刻意给两者分辨度（用户 2026-08-31 指定），这是**排版增强**而非还原
——写在这里是免得后人拿 Gate T 的 bold 计数去对，以为多出来的粗体是 bug。

开关：`structured_to_md.py --split-verse-num`（不给则沿用加尔文的一律加粗）。

### 三种易漏的编号形态（都实测踩过，漏一种就被 kramdown 变成 `<ol>`）

```
19, 20.   逗号合并
1-11.     连字符范围（含 – —）
32 33.    纯空格分隔（原书漏了逗号，1cor 一处）
```

后接字符也不能只认大写：`4. is because the church…`（跨页断句造成的小写
起首）、`2. (For he saith…`（左括号起首）都要收。最终正则：
...
验收：`grep -cE '^[0-9]+[0-9,，  –—-]*\. '` 产物里应为 0（全部已转成加粗或
enum-num），且渲染后 `<ol>`（排除脚注列表）为 0。
