#!/usr/bin/env python3
"""扫描 calvin/john/*.md 找 `<h2 class="verse-anchor" id="john-N-V">` 标记，
生成 calvin/john/verse-index/index.html 细化到节的注释索引。

每个有 Calvin 注释的经节 → 一个 pill, 链接 `/calvin/john/<ch>/#john-<ch>-<v>`,
点击直跳到注释段（不是经文块）。
...
SRC_DIR = ROOT / 'calvin' / 'john'
OUT_PATH = ROOT / 'calvin' / 'john' / 'verse-index' / 'index.html'

ANCHOR_RE = re.compile(r'<h2 class="verse-anchor"\s+id="john-(\d+)-(\d+)"')
    ...
    for ch in sorted(anchors.keys()):
        ...
        for v in verses:
            pills.append(
                ...
                f'href="{{{{ site.baseurl }}}}/calvin/john/{ch}/#john-{ch}-{v}" '
                f'title="约翰福音 {ch}:{v}">'
                f'<span class="pill-verse">{v}</span></a>'
            )
        rows.append(
            f'<div class="ch-row">\n'
            f'  <div class="ch-label">约翰福音 {ch}</div>\n'
...
<style>
  ...
  @media (max-width: 640px) {{
    .ch-row {{ flex-direction:column; gap:4px; }}
    .ch-label {{ flex:none; padding-top:0; }}
  }}
</style>
  ...
  <div class="row">
    <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
      ...
      <div class="verse-index-back">
        <a href="{{{{ site.baseurl }}}}/calvin/john/">&larr; 返回约翰福音注释</a>
      </div>
      ...
      <p class="verse-index-intro">
        ...
        点击任一胶囊按钮直接跳转到对应注释段。未列出的节号表示 Calvin
        未对该节单独作注。
      </p>
