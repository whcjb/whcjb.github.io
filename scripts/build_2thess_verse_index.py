#!/usr/bin/env python3
"""扫描 calvin/2thessalonians/*.md 提取 per-verse commentary-anchor 锚点
（`<div class="commentary-anchor" id="2thessalonians-CH-V"></div>`，由
(publish_2thess_zh add_verse_anchors_in_body) 在每个 `**N.**` 注释段前插入），
生成 calvin/2thessalonians/verse-index/index.html 经文段索引页。
...
SRC_DIR = ROOT / 'calvin' / '2thessalonians'
OUT_PATH = ROOT / 'calvin' / '2thessalonians' / 'verse-index' / 'index.html'

# 仅匹配 per-verse 锚点 id="2thessalonians-N-V"，跳过 range 锚点 id="2thessalonians-N-S-E"
# 关键：尾部 \" 之前不能再有 -，否则就是 range
PER_VERSE_RE = re.compile(
    r'<div class="commentary-anchor" id="2thessalonians-(\d+)-(\d+)"></div>'
)
...
def build_html(anchors: dict[int, list[int]]) -> str:
    ...
    for ch in sorted(anchors.keys()):
        ...
        pills: list[str] = []
        for v in verses:
            aid = f'2thessalonians-{ch}-{v}'
            pills.append(
                f'<a class="vi-pill" '
                f'href="{{{{ site.baseurl }}}}/calvin/2thessalonians/{ch}/#{aid}" '
                f'title="帖撒罗尼迦后书 {ch}:{v}">{v}</a>'
            )
        rows.append(
            f'        <div class="vi-ch">\n'
            f'          <div class="vi-ch-name" title="帖撒罗尼迦后书 {ch}">{ch}</div>\n'
...
<div class="container vi-wrap" style="padding-top: 70px;">
  <div class="row">
    <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
      ...
      <div class="vi-back">
        <a href="{{{{ site.baseurl }}}}/calvin/2thessalonians/">&larr; 返回帖撒罗尼迦后书注释</a>
      </div>

      <h1 class="vi-title">加尔文《帖撒罗尼迦后书注释》— 经文索引</h1>
