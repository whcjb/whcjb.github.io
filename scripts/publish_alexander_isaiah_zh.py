#!/usr/bin/env python3
"""alexander_raw/isaiah/zh_chapters/*.md → alexander/isaiah/zh/*.md（中文版）。

    python3 scripts/publish_alexander_isaiah_zh.py          # 发布全部已翻译的
    python3 scripts/publish_alexander_isaiah_zh.py 1 3      # 只发布指定篇
...
RAW = ROOT / 'alexander_raw/isaiah/zh_chapters'
META = ROOT / 'alexander_raw/isaiah/zh_meta.json'
OUT = ROOT / 'alexander/isaiah/zh'
EN = ROOT / 'alexander/isaiah'

BOOK_NAME_ZH = '亚历山大《以赛亚书注释》'
...
LABELS = {
    'preface': '卷一 著者序',
    'introduction': '卷一 导论',
    'later-preface': '卷二 著者序',
    'later-introduction': '卷二 导论',
}


def label(sec: str) -> str:
    return LABELS.get(sec) or f'以赛亚书 第 {sec} 章'


def sequence():
    """全书顺序：卷一的序与导论 + 1-39 章，再卷二的序与导论 + 40-66 章。

    与英文版排的是同一条线（英文 39.md 的 next_section 就是 later-preface），
    切换中英文时两边对得上。
    """
    return (['preface', 'introduction'] + [str(i) for i in range(1, 40)]
            + ['later-preface', 'later-introduction']
            + [str(i) for i in range(40, 67)])
...
def backfill_zh_url(sec: str):
    ...
    if 'zh_url:' in t.split('\n---\n', 1)[0]:
        return False
    head, body = t.split('\n---\n', 1)
    p.write_text(f'{head}\nzh_url: "/alexander/isaiah/zh/{sec}/"\n---\n{body}',
                 encoding='utf-8')
    return True
...
def main(only=None):
    if not RAW.exists():
        sys.exit(f'找不到 {RAW}，先跑 translate_alexander_isaiah.py')
    ...
    for k, sec in enumerate(names):
        ...
        fm = ['---', 'layout: alexander-chapter', 'book_id: isaiah',
        ...
        fm += [f'en_url: "/alexander/isaiah/{sec}/"', 'zh: true', '---']
        path.write_text('\n'.join(fm) + '\n\n' + body + '\n', encoding='utf-8')
        written += 1
