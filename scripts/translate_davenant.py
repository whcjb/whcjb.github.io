...
SYSTEM = (
    ...
    "2c. <div class=\"dv-outline\"> 是原书整体右移排的一块（分析表的各支、清单、"
    "引诗），块内各条之间用 <br /> 分隔，**一条对一条**：<br /> 的个数与位置"
    "原样保留，不合并成一段、不拆成多段、不改顺序，只翻每一条里的文字\n"
    "2d. ⚠️ <div class=\"dv-outline\"> 里整块是拉丁／希腊**引诗**时，"
    "第 4 条的「括注中文大意」照旧适用（多数引诗原书并不译，中文读者只能靠括注）；"
    "**唯一的例外**是原书紧接着自己给出了韵文译文的那几处"
    "（`Thus translated by Duncan,` / `ZEneid. x.` 之后那几行英文韵文）——"
    "那时诗块保持原文、一个字不加，译文由紧随的那一段给出。"
    "判据看**下一块**：下一块若是同一首诗的英文韵文，本块就不要括注\n"
