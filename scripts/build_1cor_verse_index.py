#!/usr/bin/env python3
"""扫描 calvin/1corinthians/*.md 找 `<div class="commentary-anchor" id="1corinthians-N-V">`
per-verse 锚点，生成 calvin/1corinthians/verse-index/index.html 细化到节的注释索引。

每个有 Calvin 注释的经节 → 一个 pill，链接 `/calvin/1corinthians/<ch>/#1corinthians-<ch>-<v>`，
点击直跳到注释段（不是经文块）。
...
SRC_DIR = ROOT / 'calvin' / '1corinthians'
OUT_PATH = ROOT / 'calvin' / '1corinthians' / 'verse-index' / 'index.html'

# per-verse anchor only (exclude range anchors like 1-corinthians-1-1-3 with hyphen prefix)
ANCHOR_RE = re.compile(r'<div class="commentary-anchor" id="1corinthians-(\d+)-(\d+)"></div>')
    ...
    for ch in sorted(anchors.keys()):
        ...
        for v in verses:
            pills.append(
                ...
                f'href="{{{{ site.baseurl }}}}/calvin/1corinthians/{ch}/#1corinthians-{ch}-{v}" '
                f'title="哥林多前书 {ch}:{v}">'
                f'<span class="pill-verse">{v}</span></a>'
            )
        rows.append(
            f'<div class="ch-row">\n'
            f'  <div class="ch-label">哥林多前书 {ch}</div>\n'
  ...
  <div class="row">
    <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
      ...
      <div class="verse-index-back">
        <a href="{{{{ site.baseurl }}}}/calvin/1corinthians/">&larr; 返回哥林多前书注释</a>
      </div>
