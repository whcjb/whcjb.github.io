...
def ocr_image(image_bytes: bytes, prompt: str = None) -> str:
    ...
    if prompt is None:
        prompt = (
            ...
            "⚠️ 严格转写规则（最重要）：\n"
            "1) 只输出图片上能看到的字，**禁止** 补全、推断、续写任何内容\n"
            "2) 如果段尾是不完整词（如换行处只有"保"字，下文在另一页），\n"
            "   就输出到"保"为止，**不要** 自己续写"罗"或其他猜测字\n"
            "3) 如果模糊看不清，输出 □ 占位，**不要** 凭语义猜测填字\n"
            "4) 不要扩充、不要润色、不要修正原文错别字\n"
            "\n"
            "段落与格式：\n"
            "5) 保持原文的段落分隔\n"
            "6) 去掉页码数字（如"235"独立一行）\n"
            "7) 去掉页眉页脚（如"加尔文文集"、"罗马书注释"等）\n"
            "8) 保留脚注标记（如①②等）\n"
            "9) 如果有希腊文或希伯来文原文，也保留\n"
            "\n"
