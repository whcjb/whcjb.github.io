#!/usr/bin/env python3
"""publish_haggai_zh.py — 加尔文《哈该书注释》中译发布。

共 2 章，章号与英文 calvin/haggai-en/ 对齐。

- calvin-en 布局；front matter 本地化；prev/next 只在已译章之间连。
- 已发布章保留原 date；新章用当前真实时间。
- 末尾重写 index.html（calvin-book-modern）。

用法: python3 scripts/publish_haggai_zh.py        # 发布所有已翻译
      python3 scripts/publish_haggai_zh.py 1 3    # 只发布指定章
"""
...
SRC_DIR = ROOT / 'calvin_raw/haggai/zh_chapters'
OUT_DIR = ROOT / 'calvin/haggai'
BOOK_ID = 'haggai'
BOOK_NAME = '哈该书'
TOTAL = 2
...
def main():
    ...
    for n in (['preface'] if has_preface else []) + nums:
        ...
        if n == 'preface':
            ...
            if 1 in translated:
                fm += ['next_section: 1', 'next_label: "哈该书 1"']
            fm += ['---', '']
        ...
        if n > 1 and (n - 1) in translated:
            fm += [f'prev_section: {n-1}', f'prev_label: "哈该书 {n-1}"']
        if n < TOTAL and (n + 1) in translated:
            fm += [f'next_section: {n+1}', f'next_label: "哈该书 {n+1}"']
        fm += ['---', '']
        (OUT_DIR / f'{n}.md').write_text('\n'.join(fm) + '\n' + body + '\n',
                                         encoding='utf-8')
        print(f'  published {BOOK_ID}/{n}.md  哈该书 {n}')
    if nums:
