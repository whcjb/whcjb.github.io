#!/usr/bin/env python3
"""translate_alexander_isaiah.py —— 亚历山大《以赛亚书注释》英文版 → 中文 raw。

源取**已发布的英文页** alexander/isaiah/*.md，不取 alexander_raw/.../en_chapters/：
...
输入  alexander/isaiah/{preface,introduction,later-preface,later-introduction,1..66}.md
缓存  alexander_raw/isaiah/zh_cache/<md5>.txt   ← 必须保留，重跑成本极高
输出  alexander_raw/isaiah/zh_chapters/<sec>.md ← 翻完 chmod 444
...
本卷的命根子同样是 **斜体**：Alexander 把自己对希伯来文的译文用斜体
嵌在解说里夹译夹注，原书不另设经文框。`*` 一旦丢了，读者就分不清哪句是经文、
哪句是他的解说。所以 structural_diff 把 `*` 计数当硬指标，不符即重译。

用法（项目根目录）：
    python3 -u scripts/translate_alexander_isaiah.py --dry-run --section preface
    python3 -u scripts/translate_alexander_isaiah.py --range 1-5 --resume
    python3 -u scripts/translate_alexander_isaiah.py --done --resume   # 只译已校对完的
    python3 -u scripts/translate_alexander_isaiah.py --all --resume

**先译已校对完的那几篇**（--done）：正文的 OCR 校对还在逐章推进，没校过的章
翻了要返工。哪些算校过看 alexander_raw/isaiah/PROOFREAD_STATUS.md。
"""
import argparse
import os
import re
...
# 四篇前置件。两卷各有自己的序与导论，文件名沿用英文版的。
FRONT = ['preface', 'introduction', 'later-preface', 'later-introduction']
# **已经逐句校对完**的那几篇（alexander_raw/isaiah/PROOFREAD_STATUS.md）。
# 正文的 OCR 校对还在逐章推进，没校过的章翻了要返工，所以默认先译这一批。
# 校完一章就往这里加一个。
DONE = FRONT + ['1', '12', '40']
...
SYSTEM = (
    ...
    "《以赛亚书注释》（卷一 1846 年《早期预言》，卷二 1847 年《后期预言》）。\n"
    "此书体例：先给每章一段总论，再逐节给出他自己对希伯来文的译文，"
    "随即接上语法、词义、平行经文的辨析，并逐一点评前代与同代释经家的读法。"
    "文风是十九世纪普林斯顿的学术散文，"
    ...
    "Isaiah→以赛亚，the Prophet→先知，Septuagint→七十士译本，"
    "Vulgate→武加大译本，Peshito→别西大译本，Targum→他尔根，"
    "Chaldee (Paraphrase)→迦勒底译本，Masoretic→马所拉，"
    "Uzziah→乌西雅，Jotham→约坦，Ahaz→亚哈斯，Hezekiah→希西家，"
    "Sennacherib→西拿基立，Cyrus→古列，Babylon→巴比伦，"
    "Assyria→亚述，Ephraim→以法莲，Judah→犹大\n"
    ...
    "  (二) 十九世纪德国批评学派那一串名字——他几乎每节都要点评他们的读法，"
    "中文里多半没有通行译名，读者对不上是谁："
    "希齐格（Hitzig）、克诺贝尔（Knobel）、亨德韦克（Hendewerk）、"
    "温布赖特（Umbreit）、毛雷尔（Maurer）、黑弗尼克（Hävernick）、"
    "德雷克斯勒（Drechsler）、科佩（Koppe）、贝托尔特（Bertholdt）、"
    "艾希霍恩（Eichhorn）、奥古斯提（Augusti）、洛沃特（Lowth）、"
    "亨德森（Henderson）、巴恩斯（Barnes）、克莱纳特（Kleinert）。\n"
    ...
    "Isai. xvii. 13 → 以赛亚书 17:13；Ps. xxvi. 4, 5 → 诗篇 26:4, 5；"
    ...
    "   inscription/title→题铭（书卷或段落开头那一行）\n"
    "   the earlier/later prophecies→早期／后期预言\n"
    "   higher criticism→高等批评　neological→新神学派的\n"
    "   the old dispensation→旧约的时代　covenant→约　\n"
    "11. 他特意大写的提法（如 the Holy One of Israel→以色列的圣者）"
    "全章同一译法，不要一处一样\n"
...
ANNOTATE = {
    # 近现代学者／释经家。值 = 已知的中文写法（能自动补），
    # None = 只检测不自动补（不知道模型会译成什么字，猜着补等于乱改正文）。
    'Hengstenberg': '亨斯滕伯格',
    'De Wette': '德维特',
    'Ewald': '埃瓦尔德',
    'Gesenius': '格塞尼乌斯',
...
def translate_one(text: str, resume: bool, label: str = '') -> str:
    ...
    base = ("把下面这段亚历山大《以赛亚书注释》的英文译成简体中文。"
...
def run_section(name: str, resume: bool, dry: bool, publish: bool = True) -> None:
    ...
    hit = sum(1 for t in todo
              if (CACHE / f'{tf.md5key("ALEX_ISA::" + _split_prefix(t)[1])}.txt').exists())
    ...
    cached_keys = {tf.md5key('ALEX_ISA::' + _split_prefix(u)[1]) for u in todo}
...
def autopublish() -> None:
    ...
    import subprocess
    try:
        r = subprocess.run([sys.executable, 'scripts/publish_alexander_isaiah_zh.py'],
...
def main():
    ...
    ap.add_argument('--section', help='preface / introduction / later-preface / later-introduction / 1..66')
    ap.add_argument('--range', help='章号区间，如 1-5')
    ap.add_argument('--all', action='store_true', help='四篇前置件 + 全部 66 章')
    ap.add_argument('--done', action='store_true',
                    help='只译已逐句校对完的那几篇（见 PROOFREAD_STATUS.md）')
    ...
    if a.section:
        ...
        targets = FRONT + [str(i) for i in range(1, 67)]
    elif a.done:
        targets = DONE
    else:
        ap.error('要么 --section，要么 --range，要么 --done，要么 --all')
