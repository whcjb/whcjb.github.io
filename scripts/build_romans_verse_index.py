#!/usr/bin/env python3
"""扫描 calvin/romans/*.md 找 `**罗马书 N:V。**` verse-anchor 标记，
生成 calvin/romans/verse-index/index.html 细化到节的注释索引。

每个有 Calvin 注释的经节 → 一个 pill，链接 `/calvin/romans/<ch>/#romans-<ch>-<v>`，
点击直跳到注释段（不是经文块）。
...
SRC_DIR = ROOT / 'calvin' / 'romans'
OUT_PATH = ROOT / 'calvin' / 'romans' / 'verse-index' / 'index.html'

# `**罗马书 N:V。**`  (kramdown 渲染后 calvin-en.html JS 生成 id=romans-N-V)
ANCHOR_RE = re.compile(r'\*\*罗马书\s+(\d+):(\d+)。\*\*')
...
def collect_anchors() -> dict[int, list[int]]:
    ...
    for path in sorted(SRC_DIR.glob('*.md')):
        ...
        for m in ANCHOR_RE.finditer(text):
            ch = int(m.group(1))
            v = int(m.group(2))
            # 一般 ch_file == ch；relocate 后罕见有跨章 anchor 但保险起见用 ch
            if ch == ch_file:
                out[ch].add(v)
    ...
    for ch in sorted(anchors.keys()):
        ...
        for v in verses:
            pills.append(
                ...
                f'href="{{{{ site.baseurl }}}}/calvin/romans/{ch}/#romans-{ch}-{v}" '
                f'title="罗马书 {ch}:{v}">'
                f'<span class="pill-verse">{v}</span></a>'
            )
        rows.append(
            f'<div class="ch-row">\n'
            f'  <div class="ch-label">罗马书 {ch}</div>\n'
  ...
  <div class="row">
    <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
      ...
      <div class="verse-index-back">
        <a href="{{{{ site.baseurl }}}}/calvin/romans/">&larr; 返回罗马书注释</a>
      </div>
