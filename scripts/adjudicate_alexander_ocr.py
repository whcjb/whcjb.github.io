...
# HTML 实体也要屏蔽：publish 会把正文里的野生 `<` 转成 `&lt;`（否则 kramdown
# 可能当标签解析），token 正则会把 `&lt;` 读成一个叫 `lt` 的词，凭空多出 29 个
# 可疑 token。
...
MANUAL_TEXT = [
    ...
    # 只有影像能定案。逐条对照见 logs/alexander_image_adjudicate_ps1-10.tsv。
    ...
    ('or dii-ect him',
     'or direct him'),
    ('*ivUl hear, in wj calUrKj,*',
     '*will hear, in my calling,*'),
    ('*unto hbn.*',
     '*unto him.*'),
    ('*Lafic and sin not, i. c.*',
     '*Rage and sin not, i. e.*'),
    ('Ephesians iv. 2G',
     'Ephesians iv. 26'),
    ('*Be ye anrirtj and sin not,*',
     '*Be ye angry and sin not,*'),
    ('The English word *rar/e*',
    ...
    ("God's vdll",
     "God's will"),
    ('engrossed vdth his distresses',
     'engrossed with his distresses'),
    ('*shall he all viy enemies.*',
     '*shall be all my enemies.*'),
    ('he fled fi-om Absalom',
     'he fled from Absalom'),
    ('*God is angi-y every day.*',
     '*God is angry every day.*'),
    ('*And on his oum crown',
    ...
    ('appellative denoting/ooZ,',
     'appellative denoting *fool,*'),
    ('to mean ybr *Ben,*',
     'to mean *for Ben,*'),
    ('*the Jcnowers of thy name,*',
     '*the knowers of thy name,*'),
    ('the Lord fi-om Jerusalem',
     'the Lord from Jerusalem'),
    ('is of com-se to be condemned',
     'is of course to be condemned'),
    ('seek Gx)d in prayer',
     'seek God in prayer'),
    ('in his pride ai-e, that God',
     'in his pride are, that God'),
    ('*His ivays are firm,*',
     '*His ways are firm,*'),
    ('by a plm-al proper',
     'by a plural proper'),
    ('*Ai-ise, Jehovah!',
     '*Arise, Jehovah!'),
    ('*thou vdlt seek for his wickedness*',
     '*thou wilt seek for his wickedness*'),
    ('Thou *vn^t* find nothing',
     'Thou wilt find nothing'),
    ('*ai-e nations,*',
     '*are nations,*'),
    ('*cEstus* and *cestas,*',
     '*æstus* and *æstas,*'),
    ('*oves et loves.*',
     '*oves et boves.*'),
    ("(Ps. xxx\\'i. 13, cxli. *lQ>),from their plans, i. e.,*",
     '(Ps. xxxvi. 13, cxli. 10), *from their plans, i. e.,*'),
    ('The Hebrew word 1^011, derived from IDn,',
     'The Hebrew word חסיד, derived from חסד,'),
    ('*humble* and *afflicted iP^y^J* and *Uy^y}))* is not essential',
     '*humble* and *afflicted* (ענוים and עניים) is not essential'),
    ('seems to point out ""jy •T as the subject',
     'seems to point out עני as the subject'),
    ("The adjective ''jy means afflicted",
     'The adjective עני means afflicted'),
    ("The Hebrew word *'TtD,* to which the older writers",
     'The Hebrew word תך, to which the older writers'),
    ('The word translated *sufferer (HD/tl* for ^Vh)',
     'The word translated *sufferer* (חלכה for חילך)'),
    ('*and know,* be assured,. t^a&lt; *the Lord,* Jehovah, *hath set apart,*',
     '*and know,* be assured, *that the Lord,* Jehovah, *hath set apart,*'),
    ...
    # 1864 扫描件影像判定（scripts/crop_alexander_page.py）。
    # 换的是 token 所在的**词簇**（token 加上被杂散符号分开的相邻残片），
    # 且 old 串在**全书唯一**——MANUAL_TEXT 是纯字符串替换，只取 token
    # 会命中别处的正确词（`lliou`→`Thou` 会把 rebellious 改成 rebeThous、
    # `anasseh`→`Manasseh` 会把 Manasseh 改成 MManasseh，都踩过）。
    ('vcild', 'wild'),
    ('Jcingdom', 'kingdom'),
    ('Jcinr/dom', 'kingdom'),
    ('phasure', 'pleasure'),
    ...
    ('proffc', 'proud'),
    ('Gittir', 'Gitti'),
    ('Al-muth-luhhen', 'labben'),
    ('mimor', 'mizmôr'),
    ('Imighty', 'Almighty'),
    ('sufi"ocation', 'suffocation'),
    ('v/ildernesSy', 'wilderness'),
    ('e 31anasseh, an', 'e 31Manasseh, an'),
    ('suferei-s', 'sufferers'),
    ...
    ('figui-e', 'figure'),
    ('at^reeing', 'agreeing'),
    ('fi-equently', 'frequently'),
    ('in\\isible', 'invisible'),
    ('a^.ceptance', 'acceptance'),
    ('*de facto,* n', '*de malefactors,* n'),
    ('occvirs', 'occurs'),
    ('ma,nifestation', 'manifestation'),
    ('difi"erence', 'reference'),
    ('afl&rmation', 'affirmation'),
    ('di^erence', 'reference'),
    ('combm?ii\\ons', 'combinations'),
    ('eonjunction', 'conjunction'),
    ('i-lause', 'clause'),
