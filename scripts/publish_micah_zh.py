#!/usr/bin/env python3
"""publish_micah_zh.py — 加尔文《弥迦书注释》中译发布。

共 7 章，章号与英文 calvin/micah-en/ 对齐。

- calvin-en 布局；front matter 本地化；prev/next 只在已译章之间连。
- 已发布章保留原 date；新章用当前真实时间。
- 末尾重写 index.html（calvin-book-modern）。

用法: python3 scripts/publish_micah_zh.py        # 发布所有已翻译
      python3 scripts/publish_micah_zh.py 1 3    # 只发布指定章
"""
...
SRC_DIR = ROOT / 'calvin_raw/micah/zh_chapters'
OUT_DIR = ROOT / 'calvin/micah'
BOOK_ID = 'micah'
BOOK_NAME = '弥迦书'
TOTAL = 7
...
def main():
    ...
    for n in (['preface'] if has_preface else []) + nums:
        ...
        if n == 'preface':
            ...
            if 1 in translated:
                fm += ['next_section: 1', 'next_label: "弥迦书 1"']
            fm += ['---', '']
