#!/usr/bin/env python3
"""publish_ezekiel_zh.py — 加尔文《以西结书注释》中译发布。

加尔文生前只讲到第 20 章，故 total_chapters=20，与英文 calvin/ezekiel-en/ 一致。

- calvin-en 布局；front matter 本地化；prev/next 只在已译章之间连。
- 已发布章保留原 date；新章用当前真实时间。
- 末尾重写 index.html（calvin-book-modern）。

用法: python3 scripts/publish_ezekiel_zh.py        # 发布所有已翻译
      python3 scripts/publish_ezekiel_zh.py 1 3    # 只发布指定章
"""
...
SRC_DIR = ROOT / 'calvin_raw/ezekiel/zh_chapters'
OUT_DIR = ROOT / 'calvin/ezekiel'
BOOK_ID = 'ezekiel'
BOOK_NAME = '以西结书'
TOTAL = 20
...
def main():
    ...
    for n in (['preface'] if has_preface else []) + nums:
        ...
        if n == 'preface':
            ...
            if 1 in translated:
                fm += ['next_section: 1', 'next_label: "以西结书 1"']
            fm += ['---', '']
