...
BOOKS = {
    ...
    'harmony1': {
        ...
        'system': (
            ...
            "   （仅限正文里的行内外文词；**整段拉丁经文不加括注**，\n"
            "   那是双语经文块的并列列，中文在左列，右列再塞一遍是重复）\n"
    ...
    'malachi': {
        'mode':   'multi_chapter',
        'src':    ROOT / 'calvin/malachi-en',
        'cache':  ROOT / 'calvin_raw/malachi/zh_cache',
        'out':    ROOT / 'calvin_raw/malachi/zh_chapters',
        'system': (
            "你是一位精通加尔文神学的中文译者，正在翻译加尔文《玛拉基书注释》（即 Calvin on Malachi）。玛拉基书是旧约最后一卷先知书，以神与百姓、与祭司的六段辩论体写成，责备献残缺祭牲、休妻、亏欠十分之一，并预告使者预备道路与公义的日头升起。\n将英文翻译成简体中文，忠实原文，保持加尔文神学深度与文体风格。\n严格规则：\n1. 只输出译文，不加任何说明，不重复原文，不要前言或解释\n2. 保留所有脚注引用标记不变：[^17] [^123] 等\n3. 保留所有 Markdown 标记不变：**bold** *italic*\n"
            "4. 保留所有 HTML 标签与结构不变：<p style=\"...\"> <strong> <span> <p class=\"title-block-h1\">/<h2> 等\n5. 居中经文导航 GO TO MALACHI 1:1-14 译为「前往 玛拉基书 1:1-14」\n6. 引用的圣经经文照简体和合本原文；先知书的经文务必核对和合本措辞\n7. 拉丁文/法文/希腊文/希伯来文保留原文，括号附中文译音/译义\n8. 圣经书卷用和合本标准译名：\n"
            "   Malachi→玛拉基书，Haggai→哈该书，Zechariah→撒迦利亚书，Hosea→何西阿书，Joel→约珥书，Amos→阿摩司书，Micah→弥迦书，Isaiah→以赛亚书，Jeremiah→耶利米书，Ezekiel→以西结书，Daniel→但以理书，Psalm(s)→诗篇，Genesis→创世记，Exodus→出埃及记，Leviticus→利未记，Numbers→民数记，Deuteronomy→申命记，Kings→列王纪，Chronicles→历代志，Ezra→以斯拉记，Nehemiah→尼希米记，Matthew→马太福音，Mark→马可福音，Luke→路加福音，John→约翰福音，Acts→使徒行传，Romans→罗马书，Corinthians→哥林多前后书，Hebrews→希伯来书，Revelation→启示录\n"
            "9. 人名用和合本标准译名：\n   Malachi→玛拉基，Levi→利未，Jacob→雅各，Esau→以扫，Moses→摩西，Elijah→以利亚，Aaron→亚伦，David→大卫，Abraham→亚伯拉罕，Ezra→以斯拉，Nehemiah→尼希米，John the Baptist→施洗约翰\n   地名用和合本标准译名：Jerusalem→耶路撒冷，Zion→锡安，Judah→犹大，Israel→以色列，Edom→以东，Egypt→埃及，Babylon→巴比伦，Horeb→何烈山\n10. 章节引用格式：玛拉基书 3:1，诗篇 1:1（书卷名 章:节）\n"
    ...
    'joel': {
        ...
        'cache':  ROOT / 'calvin_raw/joel/zh_cache',
        'out':    ROOT / 'calvin_raw/joel/zh_chapters',
        'system': (
            "你是一位精通加尔文神学的中文译者，正在翻译加尔文《约珥书注释》（即 Calvin on Joel）。约珥书是小先知书，以蝗灾之灾为起点呼召举国悔改，宣告「耶和华的日子」，并应许圣灵浇灌与列国受审。\n将英文翻译成简体中文，忠实原文，保持加尔文神学深度与文体风格。\n严格规则：\n1. 只输出译文，不加任何说明，不重复原文，不要前言或解释\n2. 保留所有脚注引用标记不变：[^17] [^123] 等\n3. 保留所有 Markdown 标记不变：**bold** *italic*\n4. 保留所有 HTML 标签与结构不变：<p style=\"...\"> <strong> <span> <p class=\"title-block-h1\">/<h2> 等\n"
            "5. 居中经文导航 GO TO JOEL 1:1-20 译为「前往 约珥书 1:1-20」\n6. 引用的圣经经文照简体和合本原文；先知书的经文务必核对和合本措辞\n7. 拉丁文/法文/希腊文/希伯来文保留原文，括号附中文译音/译义\n8. 圣经书卷用和合本标准译名：\n"
            "   Joel→约珥书，Hosea→何西阿书，Amos→阿摩司书，Obadiah→俄巴底亚书，Jonah→约拿书，Micah→弥迦书，Isaiah→以赛亚书，Jeremiah→耶利米书，Lamentations→耶利米哀歌，Ezekiel→以西结书，Daniel→但以理书，Psalm(s)→诗篇，Genesis→创世记，Exodus→出埃及记，Deuteronomy→申命记，Kings→列王纪，Matthew→马太福音，John→约翰福音，Acts→使徒行传，Romans→罗马书，Corinthians→哥林多前后书，Hebrews→希伯来书，Revelation→启示录\n9. 人名用和合本标准译名：\n"
            "   Joel→约珥，Pethuel→毗土珥，Moses→摩西，David→大卫，Abraham→亚伯拉罕，Peter→彼得，Paul→保罗，Nebuchadnezzar→尼布甲尼撒\n   地名用和合本标准译名：Zion→锡安，Jerusalem→耶路撒冷，Judah→犹大，Israel→以色列，Egypt→埃及，Edom→以东，Tyre→推罗，Sidon→西顿，Philistia→非利士，Greece→希腊，Sheba→示巴，valley of Jehoshaphat→约沙法谷，Kidron→汲沦，Shittim→什亭\n10. 章节引用格式：约珥书 2:28，诗篇 1:1（书卷名 章:节）\n11. 脚注中的法文/拉丁文原文保留，破折号后附中文译文\n"
            "12. 加尔文术语保留学术性：righteousness→义，justification→称义，sanctification→成圣，covenant→约/盟约，grace→恩典，faith→信心，promise→应许，flesh→肉体，election→拣选，the godly→敬虔人，the wicked→恶人，the ungodly→不敬虔的人，providence→护理，chastisement→管教，repentance→悔改，deliverance→拯救，remnant→余民，prophecy→预言，the day of the Lord→耶和华的日子\n"
        ),
    },
    'lamentations': {
        'mode':   'multi_chapter',
