#!/usr/bin/env python3
"""
publish_genesis_zh.py — 将 genesis 已翻译的中文章节发布到 calvin/genesis/

输入：calvin_raw/genesis/zh_chapters/<N>.md  (preface, 1..50)
输出：calvin/genesis/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/genesis/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_genesis_zh.py
"""
import json
import re
import subprocess
...
SRC_DIR = Path('calvin_raw/genesis/zh_chapters')
OUT_DIR = Path('calvin/genesis')
...
_genesis_chapters = next(b for b in _bible if b['abbrev'] == 'gn')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_genesis_chapters):
        return None
    chapter = _genesis_chapters[ch - 1]
...
_CN_DIGIT = ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九']


def _cn_num(n: int) -> str:
    """1..99 阿拉伯数字 → 中文（创世记 50 章需支持到十位数）。"""
    if n < 10:
        return _CN_DIGIT[n]
    if n < 20:
        return '十' + (_CN_DIGIT[n % 10] if n % 10 else '')
    tens, ones = divmod(n, 10)
    return _CN_DIGIT[tens] + '十' + (_CN_DIGIT[ones] if ones else '')


def cn_chapter(n) -> str:
    if n == 'preface':
        return '前言'
    return f'第{_cn_num(n)}章'
...
def clean_body(body: str) -> str:
    ...
    规范化经文引用后的 "&c."(etc.) 标记（等/等等/等语 → ……，全书统一）。"""
    lines = body.split('\n')
...
def main():
    ...
    for n in items:
        ...
        body = clean_body(body)  # 已含 etc.→…… 规范化（全书统一）
