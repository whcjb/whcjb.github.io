#!/usr/bin/env python3
"""
publish_1cor_zh.py — 将 1cor 已翻译的中文章节发布到 calvin/1corinthians/

输入：calvin_raw/1cor/zh_chapters/<N>.md  (preface, 1..16)
输出：calvin/1corinthians/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/1corinthians/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_1cor_zh.py
"""
import json
import re
import subprocess
...
SRC_DIR = Path('calvin_raw/1cor/zh_chapters')
OUT_DIR = Path('calvin/1corinthians')
...
_1cor_chapters = next(b for b in _bible if b['abbrev'] == '1co')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_1cor_chapters):
        return None
    chapter = _1cor_chapters[ch - 1]
...
def relocate_anchors_in_body(body: str) -> str:
    ...
    旧 1cor 用 id="1-corinthians-1-1-3" 风格（kebab）；保持。"""
    h2_re = re.compile(
