#!/usr/bin/env python3
"""
publish_2timothy_zh.py — 将 2timothy 已翻译的中文章节发布到 calvin/2timothy/

输入：calvin_raw/2timothy/zh_chapters/<N>.md  (preface, 1..5)
输出：calvin/2timothy/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/2timothy/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_2timothy_zh.py
"""
import json
import re
import subprocess
...
_2timothy_chapters = next(b for b in _bible if b['abbrev'] == '2tm')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_2timothy_chapters):
        return None
    chapter = _2timothy_chapters[ch - 1]
...
def relocate_anchors_in_body(body: str) -> str:
    ...
    旧 2timothy 用 id="1-corinthians-1-1-3" 风格（kebab）；保持。"""
    h2_re = re.compile(
