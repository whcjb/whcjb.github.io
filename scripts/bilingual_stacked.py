...
SEC_START_RE = re.compile(
    ...
    r'|(?:^##\s+\S+\s+[\d:：]+.*$)', re.M)
# 节号三种写法都要收：裸 `5.`、markdown `**5.**`、HTML `<strong>5.</strong>`。
# `_unbox()` 吐出的正是 <strong> 形式，原先只认前两种，于是拆完的项一个都
# 收不回来（哈该书重建 0 段，2026-09-03）。
ANY_VERSE_RE = re.compile(
    r'^(?:<p style="(?P<style>[^"]*)"[^>]*>)?\s*'
    r'(?:\*\*|<strong>)?\s*(?P<num>\d{1,3})\s*\.*\s*(?:\*\*|</strong>)?\s*'
    r'(?P<body>.*?)(?:</p>)?\s*$', re.S)
# 下一节节号前面可能是**中文标点**：`…行为上）；6. 你们撒的种多…`
# （哈该书 1:5-6，前视只认空格与右括号时，v6 的中文被留在 v5 的拉丁格里、
# v6 的正文格空着，2026-09-03 用户第四次指出）。一并收全角分号/逗号/句号。
