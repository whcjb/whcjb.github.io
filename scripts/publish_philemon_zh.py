#!/usr/bin/env python3
"""
publish_philemon_zh.py — 将 philemon 已翻译的中文章节发布到 calvin/philemon/

输入：calvin_raw/philemon/zh_chapters/<N>.md  (preface, 1..5)
输出：calvin/philemon/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/philemon/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_philemon_zh.py
"""
import json
import re
import subprocess
...
SRC_DIR = Path('calvin_raw/philemon/zh_chapters')
OUT_DIR = Path('calvin/philemon')
...
_philemon_chapters = next(b for b in _bible if b['abbrev'] == 'phm')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_philemon_chapters):
        return None
    chapter = _philemon_chapters[ch - 1]
