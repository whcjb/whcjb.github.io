#!/usr/bin/env python3
"""publish_zechariah_zh.py — 加尔文《撒迦利亚书注释》中译发布。

共 14 章，章号与英文 calvin/zechariah-en/ 对齐。

- calvin-en 布局；front matter 本地化；prev/next 只在已译章之间连。
- 已发布章保留原 date；新章用当前真实时间。
- 末尾重写 index.html（calvin-book-modern）。

用法: python3 scripts/publish_zechariah_zh.py        # 发布所有已翻译
      python3 scripts/publish_zechariah_zh.py 1 3    # 只发布指定章
"""
...
SRC_DIR = ROOT / 'calvin_raw/zechariah/zh_chapters'
OUT_DIR = ROOT / 'calvin/zechariah'
BOOK_ID = 'zechariah'
BOOK_NAME = '撒迦利亚书'
TOTAL = 14
...
def main():
    ...
    for n in nums:
        ...
        if n < TOTAL and (n + 1) in translated:
            fm += [f'next_section: {n+1}', f'next_label: "撒迦利亚书 {n+1}"']
        fm += ['---', '']
        (OUT_DIR / f'{n}.md').write_text('\n'.join(fm) + '\n' + body + '\n',
                                         encoding='utf-8')
        print(f'  published {BOOK_ID}/{n}.md  撒迦利亚书 {n}')
    if nums:
