#!/usr/bin/env python3
"""publish_joel_zh.py — 加尔文《约珥书注释》中译发布。

三章全译，章号与英文 calvin/joel-en/ 对齐。

- calvin-en 布局；front matter 本地化；prev/next 只在已译章之间连。
- 已发布章保留原 date；新章用当前真实时间。
- 末尾重写 index.html（calvin-book-modern）。

用法: python3 scripts/publish_joel_zh.py        # 发布所有已翻译
      python3 scripts/publish_joel_zh.py 1 3    # 只发布指定章
"""
...
SRC_DIR = ROOT / 'calvin_raw/joel/zh_chapters'
OUT_DIR = ROOT / 'calvin/joel'
BOOK_ID = 'joel'
BOOK_NAME = '约珥书'
TOTAL = 3
...
def main():
    ...
    for n in (['preface'] if has_preface else []) + nums:
        ...
        if n == 'preface':
            ...
            if 1 in translated:
                fm += ['next_section: 1', 'next_label: "约珥书 1"']
            fm += ['---', '']
