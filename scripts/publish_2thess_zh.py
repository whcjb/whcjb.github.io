#!/usr/bin/env python3
"""
publish_2thess_zh.py — 将 2thess 已翻译的中文章节发布到 calvin/2thessalonians/

输入：calvin_raw/2thessalonians/zh_chapters/<N>.md  (preface, 1..3)
输出：calvin/2thessalonians/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/2thessalonians/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_2thess_zh.py
"""
import json
import re
import subprocess
...
SRC_DIR = Path('calvin_raw/2thessalonians/zh_chapters')
OUT_DIR = Path('calvin/2thessalonians')
...
_2thess_chapters = next(b for b in _bible if b['abbrev'] == '2ts')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_2thess_chapters):
        return None
    chapter = _2thess_chapters[ch - 1]
...
def relocate_anchors_in_body(body: str) -> str:
    ...
    旧 2thess 用 id="1-corinthians-1-1-3" 风格（kebab）；保持。"""
    h2_re = re.compile(
