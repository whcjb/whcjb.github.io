#!/usr/bin/env python3
"""
publish_jude_zh.py — 将 jude 已翻译的中文章节发布到 calvin/jude/

输入：calvin_raw/jude/zh_chapters/<N>.md  (preface, 1)
输出：calvin/jude/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/jude/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_jude_zh.py
"""
import json
import re
import subprocess
...
SRC_DIR = Path('calvin_raw/jude/zh_chapters')
OUT_DIR = Path('calvin/jude')
...
_jude_chapters = next(b for b in _bible if b['abbrev'] == 'jd')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_jude_chapters):
        return None
    chapter = _jude_chapters[ch - 1]
