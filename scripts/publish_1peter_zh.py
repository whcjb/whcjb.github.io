#!/usr/bin/env python3
"""
publish_1peter_zh.py — 将 1peter 已翻译的中文章节发布到 calvin/1peter/

输入：calvin_raw/1peter/zh_chapters/<N>.md  (preface, 1..5)
输出：calvin/1peter/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/1peter/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_1peter_zh.py
"""
import json
import re
import subprocess
...
SRC_DIR = Path('calvin_raw/1peter/zh_chapters')
OUT_DIR = Path('calvin/1peter')
...
_1peter_chapters = next(b for b in _bible if b['abbrev'] == '1pe')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_1peter_chapters):
        return None
    chapter = _1peter_chapters[ch - 1]
