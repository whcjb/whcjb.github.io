#!/usr/bin/env python3
"""
publish_hebrews_zh.py — 将 hebrews 已翻译的中文章节发布到 calvin/hebrews/

输入：calvin_raw/hebrews/zh_chapters/<N>.md  (preface, 1..5)
输出：calvin/hebrews/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/hebrews/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_hebrews_zh.py
"""
import json
import re
import subprocess
...
SRC_DIR = Path('calvin_raw/hebrews/zh_chapters')
OUT_DIR = Path('calvin/hebrews')
...
_hebrews_chapters = next(b for b in _bible if b['abbrev'] == 'hb')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_hebrews_chapters):
        return None
    chapter = _hebrews_chapters[ch - 1]
