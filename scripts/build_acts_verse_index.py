#!/usr/bin/env python3
"""扫描 calvin/acts/*.md 提取 per-verse commentary-anchor 锚点
（`<div class="commentary-anchor" id="acts-CH-V"></div>`，由
scripts/add_acts_verse_anchors.py 在每个 `**N.**` 注释段前插入），
生成 calvin/acts/verse-index/index.html 经文段索引页。

每节一个胶囊，点击直接跳到该节注释段。若 Calvin 未对某节单独作注（如
28:14），则该节不显示胶囊——避免出现 "点 14 跳到 7" 的错位。
...
OUT_PATH = ROOT / 'calvin' / 'acts' / 'verse-index' / 'index.html'

# 仅匹配 per-verse 锚点 id="acts-N-V"，跳过 range 锚点 id="acts-N-S-E"
# 关键：尾部 \" 之前不能再有 -，否则就是 range
PER_VERSE_RE = re.compile(
    r'<div class="commentary-anchor" id="acts-(\d+)-(\d+)"></div>'
)
...
def build_html(anchors: dict[int, list[int]]) -> str:
    ...
    for ch in sorted(anchors.keys()):
        ...
        for v in verses:
            ...
            pills.append(
                f'<a class="vi-pill" '
                f'href="{{{{ site.baseurl }}}}/calvin/acts/{ch}/#{aid}" '
                f'title="使徒行传 {ch}:{v}">{v}</a>'
            )
        rows.append(
            f'        <div class="vi-ch">\n'
            f'          <div class="vi-ch-name" title="使徒行传 {ch}">{ch}</div>\n'
...
<div class="container vi-wrap" style="padding-top: 70px;">
  <div class="row">
    <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
      ...
      <div class="vi-back">
        <a href="{{{{ site.baseurl }}}}/calvin/acts/">&larr; 返回使徒行传注释</a>
      </div>

      <h1 class="vi-title">加尔文《使徒行传注释》— 经文索引</h1>
