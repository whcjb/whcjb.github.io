#!/usr/bin/env python3
"""publish_obadiah_zh.py — 加尔文《俄巴底亚书注释》中译发布。

共 1 章，章号与英文 calvin/obadiah-en/ 对齐。

- calvin-en 布局；front matter 本地化；prev/next 只在已译章之间连。
- 已发布章保留原 date；新章用当前真实时间。
- 末尾重写 index.html（calvin-book-modern）。

用法: python3 scripts/publish_obadiah_zh.py        # 发布所有已翻译
      python3 scripts/publish_obadiah_zh.py 1 3    # 只发布指定章
"""
...
SRC_DIR = ROOT / 'calvin_raw/obadiah/zh_chapters'
OUT_DIR = ROOT / 'calvin/obadiah'
BOOK_ID = 'obadiah'
BOOK_NAME = '俄巴底亚书'
TOTAL = 1
...
def main():
    ...
    for n in (['preface'] if has_preface else []) + nums:
        ...
        if n == 'preface':
            ...
            if 1 in translated:
                fm += ['next_section: 1', 'next_label: "俄巴底亚书 1"']
            fm += ['---', '']
