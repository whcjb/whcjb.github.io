#!/usr/bin/env python3
"""
publish_james_zh.py — 将 james 已翻译的中文章节发布到 calvin/james/

输入：calvin_raw/james/zh_chapters/<N>.md  (preface, 1..5)
输出：calvin/james/<N>.md         (覆盖旧版 CCEL 译本)
      calvin/james/index.html     (calvin-book-modern, chapters=已译数)
...
用法（项目根目录）：
    python3 scripts/publish_james_zh.py
"""
import json
import re
import subprocess
...
SRC_DIR = Path('calvin_raw/james/zh_chapters')
OUT_DIR = Path('calvin/james')
...
_james_chapters = next(b for b in _bible if b['abbrev'] == 'jm')['chapters']
...
def lookup_cuv(ch: int, v: int):
    if ch - 1 >= len(_james_chapters):
        return None
    chapter = _james_chapters[ch - 1]
